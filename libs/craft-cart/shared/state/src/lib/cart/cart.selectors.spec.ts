...
import { cartAdapter,  initialCartState } from './cart.reducer';
import * as CartSelectors from './cart.selectors';
import { CartPartialState } from './cart.state';

describe('Cart Selectors', () => {
  const ERROR_MSG = 'No Error Available';
  const getCartId = (it: CartEntity) => it.itemId;
  ...
  beforeEach(() => {
    state = {
      cart: cartAdapter.setAll(
        [createCartEntity('PRODUCT-AAA'), createCartEntity('PRODUCT-BBB'), createCartEntity('PRODUCT-CCC')],
        {
          ...initialCartState,
